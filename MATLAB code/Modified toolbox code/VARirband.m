function [INF,SUP,MED,BAR,INF2,SUP2] = VARirband(VAR,VARopt)
% =======================================================================
...
INF2 = zeros(nsteps,nvar,nvar);
SUP2 = zeros(nsteps,nvar,nvar);
...
pctg_inf2 = (100-68)/2; 
pctg_sup2 = 100 - (100-68)/2;
INF(:,:,:) = prctile(IR(:,:,:,:),pctg_inf,4);
SUP(:,:,:) = prctile(IR(:,:,:,:),pctg_sup,4);
INF2(:,:,:) = prctile(IR(:,:,:,:),pctg_inf2,4);
SUP2(:,:,:) = prctile(IR(:,:,:,:),pctg_sup2,4);
MED(:,:,:) = prctile(IR(:,:,:,:),50,4);
BAR(:,:,:) = mean(IR(:,:,:,:),4);
